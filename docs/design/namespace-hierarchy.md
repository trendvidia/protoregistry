...
| 5 | `GetNamespaceChain` RPC + handler + `Registry.GetNamespaceChain` (read-only, ungated). New `client.WithServerChain()` option auto-fetches the chain at construction time and wires ancestor Resolvers as parents (shared conn, independent refresh, propagated Close). `WithParent`/`WithFallback`/`WithGlobalFallback` docs updated to recommend `WithServerChain` for org-shared-types use cases while remaining functional for ad-hoc and test scenarios. Integration tests cover ancestor-type resolution and the root-namespace trivial case. | Low — additive; landed 2026-05-15 |
